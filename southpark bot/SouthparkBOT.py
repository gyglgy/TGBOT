...
async def send_message_with_retry(channel, message_text, file=None, reply_to=None):
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code == 48:  # Incorrect server salt
                logging.error("Неправильный серверный соль. Попробую снова...")
                await client.send_code_request(phone_number)  # Обновляем соль
            else:
                logging.error(f"Ошибка при отправке сообщения: {e}")
                raise  # Повторно выбрасываем исключение, если это не ошибка соли
    ...
    try:
        ...
        while True:
            ...
            if video_messages:
                ...
                sent_message_one = await send_message_with_retry(
                ...
                sent_message_two = await send_message_with_retry(
